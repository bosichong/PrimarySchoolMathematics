<template>
  ...
  <a-drawer title="其他程序参数设置" :width="550" :visible="visible" :body-style="{ paddingBottom: '80px' }"
    ...
    <a-space align="end">
      <a-form-item label="解题方式 ">
        <a-radio-group v-model:value="solution">
          <a-radio-button value="0">用口算解题</a-radio-button>
          <a-radio-button value="1">用竖式解题</a-radio-button>
        </a-radio-group>
      </a-form-item>
    </a-space>
...
axios.get('api_getconfigjson').then(function (res) {
  ...
  solution.value = config.solution
...
const handleCreatePSM = () => {
  ...
  // 算数符号
  let ss_a = []
  symadd(ss_a, symbols_a.value[0])
  symadd(ss_a, symbols_a.value[1])
  symadd(ss_a, symbols_a.value[2])
  symadd(ss_a, symbols_a.value[3])
  let ss_b = []
  symadd(ss_b, symbols_b.value[0])
  symadd(ss_b, symbols_b.value[1])
  symadd(ss_b, symbols_b.value[2])
  symadd(ss_b, symbols_b.value[3])
  let ss_c = []
  symadd(ss_c, symbols_c.value[0])
  symadd(ss_c, symbols_c.value[1])
  symadd(ss_c, symbols_c.value[2])
  symadd(ss_c, symbols_c.value[3])
