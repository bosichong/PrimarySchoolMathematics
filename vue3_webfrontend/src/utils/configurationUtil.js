...
export function saveConfiguration(id, name, configuration) {
  const stringValue = localStorage.getItem(key)

  const configurations = JSON.parse(stringValue)

  configurations.push({ id, name, data: configuration })

  return localStorage.setItem(key, JSON.stringify(configurations))
}
